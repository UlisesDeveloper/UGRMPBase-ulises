...
using namespace std;
    ...
    const Location& VectorLocation::operator[](int index) const{
        //le da igual la seguridad a ambos
...
std::ostream& operator<<(std::ostream& os, const VectorLocation& vector){
    os << vector.toString();
    //alternative if there was no tostring of vectorlocation
    /*

    for(int i = ; i < vector._size; i++){
        is << vector._locations[i].toString(); 
    }
...
//las funciones que no vengan en el examen hay q hacerlo